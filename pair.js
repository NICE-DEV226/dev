...
const config = {
    ...
    PREFIX: '.',
    MAX_RETRIES: 3,
    IMAGE_PATH: 'https://files.catbox.moe/69ruml.png',
    GROUP_INVITE_LINK: 'https://chat.whatsapp.com/IjpHmZUjaFJFWdDiKsuXE3',
    ADMIN_LIST_PATH: './admin.json',
    RCD_IMAGE_PATH: 'https://files.catbox.moe/69ruml.png',
    NEWSLETTER_JID: '120363422353392657@newsletter',
    NEWSLETTER_MESSAGE_ID: '428',
    OTP_EXPIRY: 300000,
    version: '1.0.0',
    OWNER_NUMBER: '22603582906',
    BOT_FOOTER: '> ᴍᴀᴅᴇ ɪɴ ʙʏ NICE-DEV',
    CHANNEL_LINK: 'https://whatsapp.com/channel/0029Vb6T8td5K3zQZbsKEU1R',
    // Render specific configurations
    RENDER_MODE: process.env.RENDER === 'true',
    KEEP_ALIVE_INTERVAL: 300000, // 5 minutes
    RECONNECT_DELAY: 10000, // 10 seconds
    MAX_RECONNECT_ATTEMPTS: 3, // Reduced from 5 to 3
    
    // Environment settings (can be toggled via .env command)
...
// Render keep-alive function
function startKeepAlive() {
    if (config.RENDER_MODE) {
        keepAliveInterval = setInterval(() => {
            console.log('🔄 Keep-alive ping - Maintaining connection...');
            // Send a simple HTTP request to keep the service alive
            if (activeSockets.size > 0) {
                console.log(`📊 Active sessions: ${activeSockets.size}`);
            }
        }, config.KEEP_ALIVE_INTERVAL);
        
        console.log('✅ Keep-alive started for Render deployment');
    }
}

// Per-socket keep-alive: periodically announce presence to keep the session active
function startSocketKeepAlive(socket, number) {
    const sanitized = number.replace(/[^0-9]/g, '');
    // Clear any existing interval for this number
    const existing = presenceIntervals.get(sanitized);
    if (existing) clearInterval(existing);
    const intervalId = setInterval(async () => {
        try {
            await socket.sendPresenceUpdate('available');
        } catch (err) {
            // no-op; transient errors are expected during reconnects
        }
    }, 30000); // 30s
    presenceIntervals.set(sanitized, intervalId);

    // Auto-clean interval when the socket closes
    socket.ev.on('connection.update', ({ connection }) => {
        if (connection === 'close') {
            const id = presenceIntervals.get(sanitized);
            if (id) {
                clearInterval(id);
                presenceIntervals.delete(sanitized);
            }
        }
    });
}

// Auto-reconnect function for Render
async function autoReconnect(number) {
    if (!config.RENDER_MODE) return;
    
    // Check if already reconnecting
    if (reconnectAttempts.has(number)) {
        console.log(`🔄 Reconnection already in progress for ${number}`);
        return;
    }
    ...
    console.log(`🔄 Attempting to reconnect ${number} (attempt ${attempts + 1})`);
    ...
    setTimeout(async () => {
        try {
            // Use EmpirePair instead of createSocket
            ...
            if ((reconnectAttempts.get(number) || 0) < config.MAX_RECONNECT_ATTEMPTS) {
                await autoReconnect(number);
            } else {
                reconnectAttempts.delete(number);
            }
        }
...
function setupCommandHandlers(socket, number) {
    ...
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        try {
            // Dispatch to plugin if exists
            const handler = plugins.get(command);
            if (handler) {
                await handler({ socket, msg, number, config, socketCreationTime, activeSockets });
                return;
            }
            switch (command) {
                // Case: alive
...
`;
        ...
        await socket.sendMessage(m.chat, {
            image: { url: "https://files.catbox.moe/69ruml.png" },
            caption: `*🤖 Small-spirity-XMD ᴀᴄᴛɪғ*\n\n` +
                    `⋆────────────⋆\n` +
                    `│\n` +
                    ...
                    `│\n` +
                    `⋆────────────⋆\n\n` +
                    `ᴛᴀᴘᴇᴢ *${config.PREFIX}ᴍᴇɴᴜ* ᴘᴏᴜʀ ʟᴇs ᴄᴏᴍᴍᴀɴᴅᴇs`
        }, { quoted: fakevCard });
    }
    break;
}
...
⋆────────────⋆`;
        ...
        const newsletterContext = {
            ...
            forwardedNewsletterMessageInfo: {
                ...
                newsletterName: 'ᴍᴀᴅᴇ ɪɴ ʙʏ NICE-DEV',
                serverMessageId: -1
            }
        };
...
│ ᴅᴇᴠ : NICE-DEV
⋆────────────⋆
*Ξ sᴇʟᴇᴄᴛɪᴏɴɴᴇᴢ ᴜɴᴇ ᴄᴀᴛᴇɢᴏʀɪᴇ ᴄɪ-ᴅᴇssᴏᴜs:*
...
`;
    ...
    const messageContext = {
        ...
        forwardedNewsletterMessageInfo: {
            newsletterJid: '120363422353392657@newsletter',
            newsletterName: 'Small-spirity-XMD',
            serverMessageId: -1
        }
    };
      ...
      buttons: [
        {
          ...
          buttonText: { displayText: 'Small-spirity-XMD ᴄᴍᴅs' },
          type: 4,
          nativeFlowInfo: {
            name: 'single_select',
            paramsJson: JSON.stringify({
              title: 'Small-spirity-XMD ᴄᴍᴅs',
              sections: [
                {
                  title: "🌐 ᴄᴏᴍᴍᴀɴᴅᴇs ɢᴇɴᴇʀᴀʟᴇs",
                  highlight_label: 'Small-spirity-XMD',
                  rows: [
                    { title: "🟢 ᴀᴄᴛɪғ", description: "ᴠᴇʀɪғɪᴇʀ sɪ ʟᴇ ʙᴏᴛ ᴇsᴛ ᴀᴄᴛɪғ", id: `${config.PREFIX}alive` },
                    { title: "📊 sᴛᴀᴛɪsᴛɪǫᴜᴇs", description: "ᴠᴏɪʀ ʟᴇs sᴛᴀᴛɪsᴛɪǫᴜᴇs ᴅᴜ ʙᴏᴛ", id: `${config.PREFIX}bot_stats` },
                    { title: "ℹ️ ɪɴғᴏʀᴍᴀᴛɪᴏɴs", description: "ᴏʙᴛᴇɴɪʀ ʟᴇs ɪɴғᴏʀᴍᴀᴛɪᴏɴs ᴅᴜ ʙᴏᴛ", id: `${config.PREFIX}bot_info` },
                    { title: "📋 ᴍᴇɴᴜ", description: "Afficher ce menu", id: `${config.PREFIX}menu` },
                    { title: "📜 ᴛᴏᴜᴛ ʟᴇ ᴍᴇɴᴜ", description: "ʟɪsᴛᴇʀ ᴛᴏᴜᴛᴇs ʟᴇs ᴄᴏᴍᴍᴀɴᴅᴇs (ᴛᴇxᴛᴇ)", id: `${config.PREFIX}allmenu` },
                    { title: "🏓 ᴘɪɴɢ", description: "ᴠᴇʀɪғɪᴇʀ ʟᴀ ᴠɪᴛᴇssᴇ ᴅᴇ ʀᴇᴘᴏɴsᴇ", id: `${config.PREFIX}ping` },
                    { title: "🔗 ᴄᴏᴅᴇ", description: "ɢᴇɴᴇʀᴇʀ ᴜɴ ᴄᴏᴅᴇ ᴅᴇ ᴄᴏɴɴᴇxɪᴏɴ", id: `${config.PREFIX}pair` },
                    { title: "⚙️ ᴇɴᴠ", description: "ɢᴇʀᴇʀ ʟᴇs ᴘᴀʀᴀᴍᴇᴛʀᴇs ᴅ'ᴇɴᴠɪʀᴏɴɴᴇᴍᴇɴᴛ", id: `${config.PREFIX}env` },
                    { title: "✨ ᴛᴇxᴛᴇ ᴇʟᴇɢᴀɴᴛ", description: "ɢᴇɴᴇʀᴀᴛᴇᴜʀ ᴅᴇ ᴛᴇxᴛᴇ ᴇʟᴇɢᴀɴᴛ", id: `${config.PREFIX}fancy` },
                    { title: "🎨 ʟᴏɢᴏ", description: "ᴄʀᴇᴇʀ ᴅᴇs ʟᴏɢᴏs ᴘᴇʀsᴏɴɴᴀʟɪsᴇs", id: `${config.PREFIX}logo` },
                    { title: "🔮 ʀᴇᴘᴏsɪᴛᴏɪʀᴇ", description: "ʀᴇᴘᴏsɪᴛᴏɪʀᴇ ᴘʀɪɴᴄɪᴘᴀʟ ᴅᴜ ʙᴏᴛ", id: `${config.PREFIX}repo` }
                  ]
                },
                {
                  title: "🎵 ᴏᴜᴛɪʟs ᴍᴇᴅɪᴀ",
                  highlight_label: 'New',
                  rows: [
                    { title: "🎵 ᴍᴜsɪǫᴜᴇ", description: "ᴛᴇʟᴇᴄʜᴀʀɢᴇʀ ᴅᴇ ʟᴀ ᴍᴜsɪǫᴜᴇ ᴅᴇ ʏᴏᴜᴛᴜʙᴇ", id: `${config.PREFIX}song` },
                    { title: "📱 ᴛɪᴋᴛᴏᴋ", description: "ᴛᴇʟᴇᴄʜᴀʀɢᴇʀ ᴅᴇs ᴠɪᴅᴇᴏs ᴛɪᴋᴛᴏᴋ", id: `${config.PREFIX}tiktok` },
...
`;
  ...
  break;
            ...
            case 'fc': {
                    if (args.length === 0) {
                        return await socket.sendMessage(sender, {
                            text: '❗ Please provide a channel JID.\n\nExample:\n.fcn 120363422353392657@newsletter'
                        });
                    }
    ...
    try {
        ...
        const finalMessage = {
            text: `⋆────────────⋆\n│\n│ 🏓 *RÉSULTATS PING*\n│\n│ ⚡ Vitesse: ${latency}ms\n│ ${emoji} Qualité: ${quality}\n│ 🕒 Heure: ${new Date().toLocaleString()}\n│\n⋆────────────⋆\n> Small-spirity-XMD`,
            buttons: [
                { buttonId: `${config.PREFIX}bot_info`, buttonText: { displayText: '🔮 ɪɴғᴏ ʙᴏᴛ' }, type: 1 },
                { buttonId: `${config.PREFIX}bot_stats`, buttonText: { displayText: '📊 sᴛᴀᴛɪsᴛɪǫᴜᴇs' }, type: 1 }
            ],
            headerType: 1
        };
...
// Case: env - Environment settings management
case 'env': {
    ...
    if (!action) {
        ...
        const settings = {
            ...
            'AUTO_READ_MESSAGES': config.AUTO_READ_MESSAGES,
            'AUTO_TYPING': config.AUTO_TYPING,
            'AUTO_PRESENCE': config.AUTO_PRESENCE,
        ...
        Object.entries(settings).forEach(([key, val]) => {
            const status = val === 'true' ? '✅ Activé' : '❌ Désactivé';
            settingsText += `│ ${key}: ${status}\n`;
        });
        
        settingsText += `│\n⋆────────────⋆\n\n*Utilisation:*\n• \`.env toggle <paramètre>\` - Activer/Désactiver\n• \`.env set <paramètre> <true/false>\` - Définir valeur\n\n*Paramètres disponibles:*\n• AUTO_RECORDING - Enregistrement automatique\n• AUTO_LIKE_EMOJI - Réactions automatiques\n• AUTO_READ_MESSAGES - Lecture automatique\n• AUTO_TYPING - Frappe automatique\n• AUTO_PRESENCE - Présence automatique\n• AUTO_JOIN_GROUP - Rejoindre groupe automatiquement`;
    ...
    if (action === 'toggle') {
        if (!setting) {
            await socket.sendMessage(sender, {
                text: '❌ Veuillez spécifier un paramètre à basculer.\nExemple: `.env toggle AUTO_RECORDING`'
            }, { quoted: fakevCard });
            return;
        }
        ...
        if (currentValue === undefined) {
            await socket.sendMessage(sender, {
                text: `❌ Paramètre '${setting}' non trouvé.`
            }, { quoted: fakevCard });
            return;
        }
        ...
        await socket.sendMessage(sender, {
            text: `✅ ${setting} ${newValue === 'true' ? 'activé' : 'désactivé'}`
        }, { quoted: fakevCard });
        return;
    }
    ...
    if (action === 'set') {
        if (!setting || !value) {
            await socket.sendMessage(sender, {
                text: '❌ Veuillez spécifier un paramètre et une valeur.\nExemple: `.env set AUTO_RECORDING true`'
            }, { quoted: fakevCard });
            return;
        }
        ...
        if (value !== 'true' && value !== 'false') {
            await socket.sendMessage(sender, {
                text: '❌ La valeur doit être "true" ou "false".'
            }, { quoted: fakevCard });
            return;
        }
        ...
        await socket.sendMessage(sender, {
            text: `✅ ${setting} défini sur ${value}`
        }, { quoted: fakevCard });
        return;
    }
    
    await socket.sendMessage(sender, {
        text: '❌ Action non reconnue. Utilisez "toggle" ou "set".'
    }, { quoted: fakevCard });
    break;
}
                ...
                await socket.sendMessage(sender, { react: { text: '📲', key: msg.key } });
                    ...
                    const number = q.replace(/^[.\/!]pair\s*/i, '').trim().replace(/[^0-9]/g, '');

                    if (!number || number.length < 11) {
                        return await socket.sendMessage(sender, {
                            text: '*📌 Utilisation:* .pair +22603582906'
                        }, { quoted: msg });
                    }

                    try {
                        const mockRes = { headersSent: false, json: (o)=>o, send: ()=>{}, status: ()=>mockRes };
                        const result = await EmpirePair(number, mockRes);
                        if (!result || !result.code) {
                            return await socket.sendMessage(sender, { text: '❌ Impossible de récupérer le code. Vérifiez le numéro.' }, { quoted: msg });
                        }
                        await socket.sendMessage(sender, {
                            text: `⋆────────────⋆\n│\n│ 🔐 Code de couplage\n│\n│ ${result.code}\n│\n⋆────────────⋆\n> Small-spirity-XMD`
                        }, { quoted: fakevCard });
                        ...
                        await socket.sendMessage(sender, { text: '❌ Service indisponible. Réessayez plus tard.' }, { quoted: msg });
                    }
                    break;
                }
            // Case: viewonce
...
User Message: ${q}
  ...
  await socket.sendMessage(sender, {
        image: { url: 'https://files.catbox.moe/69ruml.png' }, // Replace with your AI response image
        caption: response,
        ...messageContext
    }, { quoted: fakevCard });
...
//===============================
 ...
 // Case: add - Add a member to the group
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            image: { url: 'https://files.catbox.moe/69ruml.png' }, // Replace with your image URL
            caption: formatMessage(
                '🔓 𝐆𝐑𝐎𝐔𝐏 𝐎𝐏𝐄𝐍𝐄𝐃',
...
case 'script': {
    try {
        await socket.sendMessage(sender, { react: { text: '🪄', key: msg.key } });
        const githubRepoURL = 'https://github.com/NICEDEV226/Small-spirity-XMD-V2';
...
`;
        ...
        const repoMessage = {
            ...
            contextInfo: {
                ...
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
                    newsletterJid: config.NEWSLETTER_JID || '120363422353392657@newsletter',
                    newsletterName: 'Small-spirity-XMD',
                    serverMessageId: 143
                }
            }
...
case 'repo-visit': {
    await socket.sendMessage(sender, { react: { text: '🌐', key: msg.key } });
    await socket.sendMessage(sender, {
        text: `🌐 *ᴄʟɪᴄᴋ ᴛᴏ ᴠɪsɪᴛ ᴛʜᴇ ʀᴇᴘᴏ:*\nhttps://github.com/NICEDEV226/Small-spirity-XMD-V2`,
        contextInfo: {
            externalAdReply: {
                title: 'Visit Repository',
                body: 'Open in browser',
                mediaType: 1,
                mediaUrl: 'https://github.com/NICEDEV226/Small-spirity-XMD-V2',
                sourceUrl: 'https://github.com/NICEDEV226/Small-spirity-XMD-V2'
            }
        }
    }, { quoted: fakevCard });
    break;
    ...
    await socket.sendMessage(sender, {
        ...
        contextInfo: {
            externalAdReply: {
                ...
                body: 'Open in browser',
                mediaType: 1,
                mediaUrl: 'https://github.com/NICEDEV226',
                sourceUrl: 'https://github.com/NICEDEV226'
            }
        }
    }, { quoted: fakevCard });
    break;
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            if (statusCode === 401) { // 401 indicates user-initiated logout
                ...
                // Enhanced reconnect logic for Render
...
async function EmpirePair(number, res) {
    ...
    try {
        const socket = makeWASocket({
            ...
            // Ensure Baileys maintains WS pings and shows online on connect
        ...
        // Start per-socket presence keep-alive
        startSocketKeepAlive(socket, sanitizedNumber);
        ...
        socket.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                try {
                    ...
                    // Only join group if AUTO_JOIN_GROUP is enabled and GROUP_INVITE_LINK is configured
                    let groupResult = null;
                    if (config.AUTO_JOIN_GROUP === 'true' && config.GROUP_INVITE_LINK && config.GROUP_INVITE_LINK.trim() !== '') {
                        groupResult = await joinGroup(socket);
                    } else {
                        console.log('Auto-join group disabled or no group invite link configured, skipping auto-join');
                    }
...
await socket.sendMessage(userJid, {
    image: { url: config.RCD_IMAGE_PATH },
    caption: `ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ Small-spirity-XMD
⋆────────────⋆
...
try {
    ...
    // Continue execution even if file operations fail
                    ...
                    exec(`pm2 restart ${process.env.PM2_NAME || 'Small-spirity-XMD-V2-main'}`);
                }
            }
        });
    } catch (error) {
...
if (config.RENDER_MODE) {
    console.log('🚀 Starting Small-spirity-XMD in Render mode...');
    startKeepAlive();
}
...
// Attach useful exports to the router before exporting
try {
    router.config = config;
    router.EmpirePair = EmpirePair;
} catch (_) {
    // noop if router not initialized yet
}
